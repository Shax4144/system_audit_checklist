...
import {
  ...
  useMultipleArchiveChecklistsMutation,
...
const FormDashboard = () => {
  ...
  const [bulkDeleteChecklists, { isLoading: isBulkDeleting }] =
    useMultipleArchiveChecklistsMutation();
  ...
  const handleConfirmDelete = async () => {
    try {
      const response = await bulkDeleteChecklists(selectedChecklists).unwrap();
      appToast.success(
        "Checklists Deleted",
        response?.message ??
      ...
      appToast.error(
        "Error",
        error?.message ?? "An error occurred while deleting checklists.",
      );
    }
  };
      ...
      {isFetching ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-3 xl:grid-cols-6 gap-2">
          ...
          {checklist.map((item, index) => {
            const isSelected = selectedChecklists.includes(item.id);
            ...
            return (
              <div key={item.id} className="relative">
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex flex-wrap gap-1">
                      <p className="text-[10px] text-muted-foreground mt-1">
                        Created: <strong>{formatDate(item.created_at)}</strong>
                      </p>
                      <p className="text-[10px] text-muted-foreground mt-1">
                        Updated: <strong>{formatDate(item.updated_at)}</strong>
                      </p>
                    </div>
                  </div>
                  ...
                  >
                    {STATUS_LABEL[item.status] ?? STATUS_LABEL.draft}
                  </Badge>
                </button>
