    ...
    <div className="report-pdf-content">
      ...
      <header className="pdf-header">
        <div className="flex flex-col items-end justify-end gap-1.5">
          ...
          <span className="text-sm font-medium">REF. NO.: {info.reference_no ?? "-"}</span>
        </div>
      </header>

      <footer className="pdf-footer">
        <div className="text-[10px] text-muted-foreground text-left leading-relaxed">
      ...
      <main className="pdf-content">
        ...
        <div className="pdf-section pt-24 pb-20">
          ...
          <table className="w-full border text-sm mt-6">
            <tbody>
              <InfoRow label="Name of Establishment/Supplier" value={info.supplier} />
              <InfoRow label="Address" value={info.address} />
              <InfoRow label="Date of Audit" value={formatDate(info.auditDate)} />
              <InfoRow label="Contact Person/Authorized Representative" value={joinOrDash(info.contactPerson)} />
              <InfoRow label="Contact Number" value={joinOrDash(info.contactNumber)} />
              <InfoRow label="Products/Supplied" value={joinOrDash(info.products)} />
              ...
              <InfoRow
                ...
                value={
                  Array.isArray(info.auditCriteria) && info.auditCriteria.length ? (
                    <ul className="list-disc list-inside">
                      {info.auditCriteria.map((c, i) => <li key={i}>{c}</li>)}
                    </ul>
                  ) : "-"
                }
              />
        ...
        <div className="pdf-section pdf-section--break-before pt-24">
          ...
          <p className="mb-4">
            Overall Rating: <span className="font-semibold">{overallPercentage}%</span>
          </p>
            ...
            return (
              <div key={rating} className="mb-4 pb-3">
                ...
                <div className="pl-4 flex flex-col gap-2">
                  {group.map((entry, i) => {
                    const remarks = entry.items.filter((item) => item.remarks).map((item) => item.remarks)
                    return (
                      <div key={i} className="finding-entry">
                        <p>{i + 1}. {entry.sectionName}</p>
                        <ul className="list-disc list-inside pl-4">
                          {remarks.map((remark, j) => <li key={j}>{remark}</li>)}
                        </ul>
                      </div>
                    )
                  })}
        ...
        <div className="pdf-section pdf-section--break-before pt-24 pb-24">
          ...
          <p className="font-semibold mb-1 mt-4">Supplier's Corrective Actions:</p>
          <p className="whitespace-pre-wrap">{context.corrective_actions || "-"}</p>

          <p className="font-semibold mb-1 mt-4">Follow up Audit:</p>
          <p className="whitespace-pre-wrap">{context.follow_up_audit || "-"}</p>
          ...
          <div className="flex flex-col gap-4 pl-1">
            ...
            {observers.map((observer, i) => (
              <div key={observer.id ?? i}>
                <p>Observer {i + 1}:</p>
                <p className="pl-4">Name and signature: {observer?.full_name ?? "-"}</p>
              </div>
            ))}
          </div>
          ...
          <p className="mt-4">
            ...
            <span className="font-medium">{formatDate(context.submission_date)}</span>
          </p>
...
const InfoRow = ({ label, value }) => (
  <tr className="info-row border">
    <td className="pl-1.5 py-1.5 pr-4 font-medium align-top whitespace-nowrap w-1/3 border-r">
      {label}:
    </td>
